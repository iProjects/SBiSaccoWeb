...
namespace SBiSaccoWeb.UI.MVC.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register(RegisterModel model)
        {
            ...
            {
                // Attempt to register the _user
                try
                {
                    WebSecurity.CreateUserAndAccount(model.UserName, model.Password);
                    WebSecurity.Login(model.UserName, model.Password);

                    spUser returnedUser = CreateUser(model);

                    Session["UserId"] = null;
                    Session["UserId"] = returnedUser.Id;

                    return RedirectToAction("UploadUserPhoto");
                }
        ...
        private spUser CreateUser(RegisterModel model)
        {
            ...
            _user.DateJoined = DateTime.Today;
        ...
        [HttpPost]
        public ActionResult UploadUserPhoto(HttpPostedFileBase HttpPostedPhoto)
        {
            AdministratorComponent ac = new AdministratorComponent();
            ...
            int _userid = int.Parse(_Sessionuserid);

            spUser _User = ac.GetUserById(_userid);
            _User.Photo = fileName;
            ac.UploadUserPhoto(_User);
        ...
        public ActionResult EditProfile()
        {
            ...
            AdministratorComponent ac = new AdministratorComponent();
            spUser model = ac.GetUserByEmail(email);
            if (model == null)
            {
                throw new ArgumentNullException("", "Error Retrieving User Info.");
            }
        ...
        [HttpPost]
        public ActionResult EditProfile(spUser model)
        {
            AdministratorComponent ac = new AdministratorComponent();
            spUser _user = ac.GetUserById(model.Id);
            _user.Surname = model.Surname;
            _user.OtherNames = model.OtherNames;
            _user.InformBy = model.InformBy;
            _user.Gender = model.Gender;
        ...
        public ActionResult EditUserPhoto(int id)
        {
            Session["UserId"] = null;
            Session["UserId"] = id;
        ...
        [HttpPost]
        public ActionResult EditUserPhoto(HttpPostedFileBase HttpPostedPhoto)
        {
            AdministratorComponent ac = new AdministratorComponent();
        ...
        public ActionResult Disassociate(string provider, string providerUserId)
        {
            ...
            // Only disassociate the account if the currently logged in _user is the owner
            if (ownerAccount == User.Identity.Name)
            {
                // Use a transaction to prevent the _user from deleting their last login credential
        ...
        public ActionResult ExternalLoginCallback(string returnUrl)
        {
            ...
            {
                // If the current _user is logged in add the new account
        ...
        public ActionResult ExternalLoginConfirmation(RegisterExternalLoginModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                // Insert a new _user into the database
